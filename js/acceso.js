 ...
 * Formulario de Acceso para Trabajadores
 */
...
/**
 * Valida el campo de usuario (correo o número de empleado)
...
function validarCampoUsuario(campo) {
    ...
    const esCorreo = regexCorreoInstitucional.test(valor);
    // Validar si es número de empleado
    const esNumEmpleado = regexNumeroEmpleado.test(valor);

    if (!esCorreo && !esNumEmpleado) {
        mostrarError(campo, 'Ingrese un correo institucional (@ipn.mx) o número de empleado válido');
        return false;
    }
